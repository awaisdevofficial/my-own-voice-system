...
-- Drop policies first (if RLS is used)
DO $$
DECLARE
  r RECORD;
BEGIN
...
-- Drop triggers that use our function
DROP TRIGGER IF EXISTS set_updated_at ON users;
DROP TRIGGER IF EXISTS set_updated_at ON user_settings;
DROP TRIGGER IF EXISTS set_updated_at ON agents;
DROP TRIGGER IF EXISTS set_updated_at ON voice_profiles;
DROP TRIGGER IF EXISTS set_updated_at ON user_telephony_config;
...
DROP TABLE IF EXISTS user_telephony_config CASCADE;
...
-- =========================================================
-- PART 3: TRIGGER FUNCTION (updated_at)
-- =========================================================
...
-- Per-user settings (Twilio + SIP / LiveKit) — used by Settings UI and outbound calls
...
-- Per-user telephony config (encrypted credentials) — used by telephony connect/disconnect and agent worker
CREATE TABLE user_telephony_config (
  id                        UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id                   UUID NOT NULL UNIQUE REFERENCES users(id) ON DELETE CASCADE,
  twilio_account_sid        VARCHAR,
  twilio_auth_token         VARCHAR,
...
CREATE INDEX ix_user_telephony_config_user_id ON user_telephony_config (user_id);
CREATE INDEX ix_user_telephony_config_assigned_agent_id ON user_telephony_config (assigned_agent_id);

CREATE TRIGGER set_updated_at
  BEFORE UPDATE ON user_telephony_config
  FOR EACH ROW EXECUTE PROCEDURE set_updated_at();
...
-- =========================================================
-- PART 7: OPTIONAL RLS (uncomment if using Supabase client direct access)
-- =========================================================
...
-- ALTER TABLE user_telephony_config ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE webhooks ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE api_keys ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE knowledge_bases ENABLE ROW LEVEL SECURITY;
-- ALTER TABLE voice_profiles ENABLE ROW LEVEL SECURITY;
--
-- CREATE POLICY "Users read own" ON users FOR SELECT USING (auth.uid()::text = clerk_id OR clerk_id = current_setting('app.clerk_id', true));
-- CREATE POLICY "Agents by user" ON agents FOR ALL USING (user_id IN (SELECT id FROM users WHERE clerk_id = current_setting('app.clerk_id', true)));
-- (Repeat for other tables; app uses backend API + Clerk so RLS is optional.)
